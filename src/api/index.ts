  ...
  return useMutation<any, Error>((addVariable) => axios.post('/variables', addVariable),
    {
      ...
      onSuccess: (data) => {
        const prevData = queryClient.getQueryData('variables') as VariablesData;
...
export function useDeleteVariable(variableKey: Variable['key']) {
  const queryClient = useQueryClient();
  return useMutation(() => axios.delete(`/variables/${variableKey}`), {
    onMutate: async () => {
      await queryClient.cancelQueries('variables');
    },
    onSuccess: () => {
      const prevData = queryClient.getQueryData('variables') as VariablesData;
      const nextData = {
        variables: prevData.variables.filter((v) => v.key !== variableKey),
        totalEntries: prevData.totalEntries - 1,
      };
      queryClient.setQueryData<VariablesData>('variables', nextData);
    },
  });
